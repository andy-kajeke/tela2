...
public class SyncSupervisorMaterialRequestUploadWorker extends Worker {
    SchoolMaterialRequestsDao schoolMaterialRequestsDao;
    public SyncSupervisorMaterialRequestUploadWorker(@NonNull Context context, @NonNull WorkerParameters workerParams) {
    ...
    public Result doWork() {
        List<SchoolMaterialRequests> schoolMaterialRequests = schoolMaterialRequestsDao.getSupervisorRequestRecords(false);
        for(SchoolMaterialRequests schoolMaterialRequests1: schoolMaterialRequests) {
            ...
            try {
                ...
                if (resp == Urls.DID_WORK) {
                    schoolMaterialRequests1.setUploadedSupervisor(true);
